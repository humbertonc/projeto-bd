...
class PurchaseTable:
    ...
    def read_by_id(self, id_purchase):
        ...
        # Select purchase
        data_purchase = self.cur.execute(f"""SELECT id_cliente, forma_de_pagamento, data_compra FROM compra 
        ...
        if not ret_vals:
            ...
            for row_snack in snack_vals:
                data_price = self.cur.execute(f"SELECT preco FROM produto WHERE id_produto == {row_snack[0]}")
    ...
    def read_by_client(self, id_client):
        ...
        data_client = self.cur.execute(f"""SELECT id_compra, forma_de_pagamento, data_compra FROM compra 
